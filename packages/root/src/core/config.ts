...
export interface RootUserConfig {
  ...
  elements?: {
    ...
    /**
     * Custom elements to exclude from the SSG output. Matching elements are
     * not bundled, their assets are not emitted to `dist/html`, and their
     * `<script>` and `<link rel="stylesheet">` tags are not auto-injected into
     * pre-rendered pages.
     *
     ...
     * elements, which makes this useful for elements that only make sense when
     * a page is rendered on demand (help overlays, internal tools, etc.) and
     * shouldn't ship to a static site.
     *
     * Note that this excludes the element's assets, not its HTML. A
     * `<debug-panel>` tag rendered by a route or template is still present in
     * the SSG output, it just never upgrades into a custom element.
     *
     * Unlike `exclude`, which matches file paths and applies to every command,
     * this is matched against the element's tag name.
     *
     * @example
     * ```ts
