...
/**
 * Resolved module id for the schema registry virtual module generated by
 * @blinkk/root's pods vite plugin (`\0` + `virtual:root/schemas`, per the
 * rollup convention for resolved virtual ids).
 */
const RESOLVED_VIRTUAL_SCHEMAS_ID = '\0virtual:root/schemas';

/**
 * Minimal structural type for the vite dev server captured from the root.js
 * dev server. Kept structural to avoid a runtime/type dependency on vite.
 */
...
export function cmsPlugin(options: CMSPluginOptions): CMSPlugin {
  ...
  /**
   * The running vite dev server, captured in `configureServer()`. Used by the
   * schema watcher to regenerate `root-cms.d.ts` through the dev server's own
   * module graph instead of booting a standalone vite server per change.
   */
  let devViteContext: {viteServer: DevViteServer; rootDir: string} | null =
    null;
  ...
  const plugin: CMSPlugin = {
    ...
    configureServer: async (
      ...
      // Capture the vite dev server for the schema watcher below.
      if (serverOptions.type === 'dev') {
        const viteServer = server.get('viteServer') as DevViteServer | null;
        if (viteServer) {
          devViteContext = {
            viteServer,
            rootDir: serverOptions.rootConfig.rootDir,
          };
        }
      }
  ...
  if (process.env.NODE_ENV === 'development' && options.watch !== false) {
    // Regenerates `root-cms.d.ts` when a schema file changes.
    //
    // The project module is loaded through the running vite dev server
    // (captured in `configureServer()`) rather than through `generateTypes()`,
    // which boots a standalone vite server per call via `viteSsrLoadModule()`.
    // Repeatedly booting servers in the dev process leaks memory on every
    // schema save: the throwaway server is not fully released after `close()`
    // and `loadRootConfig()` re-bundles `root.config.ts` into node's permanent
    // ESM cache. On projects with a large number of schema files this leaked
    // several MB per save, eventually OOM-ing the dev server; it also thrashed
    // the dev server's dep optimizer cache (both servers share the project's
    // `node_modules/.vite`), causing spurious "optimized dependencies changed"
    // full reloads. Going through the dev server's own module graph re-executes
    // only the modules invalidated by the change, so regens are cheap.
    let regenInFlight: Promise<void> | null = null;
    let regenQueued = false;
    const regenerateTypes = () => {
      const ctx = devViteContext;
      if (!ctx) {
        // Dev server hasn't finished starting up; the startup flow (e.g.
        // `root-cms generate-types`) is responsible for the initial types.
        return;
      }
      if (regenInFlight) {
        // Coalesce bursts of saves into a single trailing regen.
        regenQueued = true;
        return;
      }
      regenInFlight = (async () => {
        // Invalidate the schema registry so added/removed schema files are
        // picked up (edits to existing files are invalidated by vite itself).
        const schemasModule = ctx.viteServer.moduleGraph.getModuleById(
          RESOLVED_VIRTUAL_SCHEMAS_ID
        );
        if (schemasModule) {
          ctx.viteServer.moduleGraph.invalidateModule(schemasModule);
        }
        // Avoid loading the `generate-types.js` module if it is not needed.
        // See: https://github.com/blinkk/rootjs/issues/426
        const [generateTypesModule, project] = await Promise.all([
          import('../cli/generate-types.js'),
          ctx.viteServer.ssrLoadModule(path.resolve(__dirname, './project.js')),
        ]);
        await generateTypesModule.generateTypesFromProject(
          project as ProjectModule,
          ctx.rootDir
        );
      })()
        .catch((err) => {
          // A save mid-edit can leave a schema file temporarily unparseable;
          // log the error and pick the change up on the next save.
          console.error('[root cms] failed to regenerate root-cms.d.ts:');
          console.error(err);
        })
        .finally(() => {
          regenInFlight = null;
