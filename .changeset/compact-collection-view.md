...
The compact table's column headers (ID, Title, Created, Modified) are clickable to change the sort order (kept in sync with the sort dropdown, which gains matching Title and ascending-modified options), and the table scrolls horizontally on narrow viewports. Reloads (e.g. after changing the sort) now dim the existing rows with a spinner overlay instead of blanking the page. Created/Modified cells and the doc status badges share a new `UserActionTooltip` that shows the acting user's avatar with the timestamp.
